...
extension SchemaV2 {
    ...
    final class Routine {
        ...
        struct GenericSets: Codable, Hashable, Equatable {
            ...
            var sets: [GenericSet]
            /// Links this exercise to its library entry. Nil only for pre-V2 data
            /// that predates the backfill.
            var exerciseID: UUID? = nil
            
            init(name: String = "", dataType: DataType = .repWeight, sideType: SideType? = nil, sets: [GenericSet] = [], exerciseID: UUID? = nil) {
                self.name = name
        ...
        struct RepeaterSets: Codable, Hashable, Equatable {
            ...
            init(tag: String = "", timeOn: Int = 7, timeOff: Int = 3, sets: [RepeaterSet] = [], exerciseID: UUID? = nil) {
                self.tag = tag
                self.timeOn = timeOn
        ...
        struct MaxHangSets: Codable, Hashable, Equatable {
            ...
            init(tag: String = "", isSingleArm: Bool = true, sets: [MaxHangSet] = [], exerciseID: UUID? = nil) {
                self.tag = tag
        ...
        struct CampusSets: Codable, Hashable, Equatable {
            ...
            init(type: Exercise = .maxLadder, sets: [PlannedSet] = [], exerciseID: UUID? = nil) {
                self.type = type
                self.sets = sets
                self.exerciseID = exerciseID
            }
